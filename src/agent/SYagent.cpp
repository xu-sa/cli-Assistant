...
json sagtlib::Agent::run_tool(size_t index,json* data){
    ...
    if(this->profile.working_count==6)return{{"role","tool"},{"content","You Have Been Continually Request for Tool for too many times,report to user the situation and ask for next move Instantly"},{"tool_call_id",(*data)["id"]}};
    if(this->profile.fail_count==4)return{{"role","tool"},{"content","Tool keeps Failing , you may choose to report to user and ask for next step"},{"tool_call_id",(*data)["id"]}};      
    ...
    try{
        ...
        this->profile.working_count+=1;
        this->profile.fail_count=0;
    }
    catch(const std::exception& e){
        string res = std::string(e.what());
        result["content"]=res;
        this->profile.working_count=0;
        this->profile.fail_count+=1;
    };
...
string sagtlib::Agent::send(){
    ...
    if (agent_reply["code_"] == 200) {
        ...
        if (agent_reply.contains("tool_calls")){//if there is tool usage
            this->profile.chat_state=WORKING;
            this->handle_tool_request(&agent_reply);
            return (reply_context.empty()?(this->profile.name+" : "+"working on using tool.."):message_reply);
...
void sagtlib::Agent::load(){
    ...
    for (int i = 0; i < 5; i++) {
        ...
        this->input_pool[i].type=CLT_input;
        this->input_pool[i].image="";
    }
    this->profile.chat_state = READY;
    this->profile.working_count = 0;
    this->profile.fail_count = 0;
    if(this->message_pool.empty())this->message_pool.push_back({{"role","system"},{"content",("**your name**:"+this->profile.name+"\n **who you are**:"+this->profile.whoyouare)}});
    else{
        this->message_pool[0]["role"]="system";
        this->message_pool[0]["content"]=("**your name**:"+this->profile.name+"\n **who you are**:"+this->profile.whoyouare);
    }
    
}

void sagtlib::Agent::save(const string& choice){
    json save;
    if(choice=="h"){//save chat history
        save=this->message_pool;
        handle_save(&save,this->home,this->room,(get_time()+"chat.json"));
    }
        ...
        handle_save(&save,this->home,this->room,"profile.json");
    }else cout<<"Save: no such option\n";
}

void sagtlib::Agent::start_thread(){
    this->stop_thread();
    {
    ...
    this->on=true;
    this->monitoring=thread(&sagtlib::Agent::listen_input_thread,this);
}

void sagtlib::Agent::stop_thread(){
    this->on = false;
    if (this->monitoring.joinable()) {
...
sagtlib::Agent::Agent(const string& home,const string& room):home(home),room(room){
    this->load();
    this->start_thread();
    cout<<"——Initiated agent "<<this->profile.name<<endl;
}
