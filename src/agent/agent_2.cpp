...
using namespace std;
#define SEND_IN_ONCE(agent,data,socket)  agent->respond_socket(data,1,socket);agent->respond_socket("",0,socket);

void sagtlib::Agent::push_input_(agent_input* a){
    sleep_2(2)
    if(a->client_id==""){
        SEND_IN_ONCE(this,MES_2_0,a->client_fd)
        return;
    }
    if(this->queued_input==10){
        if(a->client_fd!=-1){SEND_IN_ONCE(this,MES_2_1,a->client_fd)}
        else MES_2_0;
        return;
    };
...
void sagtlib::Agent::push_input(int i,const string& client_id,const string& text,const string& image){//input port
    ...
    this->input_pool[this->push_in].client_fd=i;//client socket number , -1 for local terminal
    this->push_in=(this->push_in+1)%INPUT_POOL_SIZE;
    this->queued_input+=1;
}
...
void sagtlib::Agent::listen_input(){//main loop thread
    while(this->on){
        ...
        this->input_pool[this->push_out].client_fd=0;
